        ...
        {items.map((item, index) => {
            ...
            return <motion.article 
               ...
               <Link href= {`/vehicles/${slug}`}> <Image 
                ...
                /></Link>
                <Heading level={3}><Link href= {`/vehicles/${slug}`}>{title}</Link></Heading>
                <Paragraph>Starting at <strong>{convertPriceToFormattedString(vehicleInformation.trimLevels[0].msrp)}</strong></Paragraph>
            </motion.article>
        })}
